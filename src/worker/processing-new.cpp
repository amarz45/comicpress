...
vips::VImage get_vips_img_from_pdf_page(
    ...
    auto buf_size = static_cast<size_t>(width) * static_cast<size_t>(height)
                  * static_cast<size_t>(bands);
    auto buf = malloc(buf_size);
    if (!buf) {
        throw std::runtime_error(
            "PDFium: Failed to allocate bitmap buffer for page "
            + std::to_string(page_number)
        );
    }

    FPDF_BITMAP bitmap
        = FPDFBitmap_CreateEx(width, height, colour_mode, buf, stride);
    if (!bitmap) {
        free(buf);
        throw std::runtime_error(
    ...
    // Fill with white background and render.
    FPDFBitmap_FillRect(bitmap, 0, 0, width, height, 0xFFFFFFFF);
    FPDF_RenderPageBitmap(bitmap, page, 0, 0, width, height, 0, render_flags);
    FPDFBitmap_Destroy(bitmap);

    auto img = vips::VImage::new_from_memory_steal(
        buf, buf_size, width, height, bands, VIPS_FORMAT_UCHAR
    );
